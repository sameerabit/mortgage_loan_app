...
abstract class StripeBase extends FormElement {
  ...
  public static function processStripe(&$element, FormStateInterface $form_state, &$complete_form) {
    ...
    // If the element is disabled render simple input with a placeholder.
    if(!empty($element["#disabled"])) {
      $element['#theme'] = 'input__textfield';
      $element['#attributes']['type'] = 'text';
      $element['#attributes']['size'] = 60;
      $element['#attributes']['placeholder'] = t('0000 0000 0000 0000  MM/YY CVC');
      return $element;
    }
    ...
    $element['stripe']['actions']['update'] = [
      '#type' => 'submit',
      '#value' => t('Update'),
      // We specifically don't use a validate ajax callback as we want to act
      // only when the whole form validation succeeds, not otherwise.
      '#submit' => [[static::class, 'submitStripeElementCallback']],
      '#ajax' => [
        'callback' => [static::class, 'ajaxStripeElementCallback'],
        'progress' => ['type' => 'none'],
      ],
    ...
    array_unshift($element['stripe']['#element_validate'], [static::class, 'validateStripeElement']);
  ...
  public static function ajaxStripeElementCallback(array $form, FormStateInterface $form_state) {
    ...
    $additional_settings = $event->getSettings();
